...
function upDown1n1(n: number) : number[] {
  ...
  const descending = ascending.slice(0, n - 1).reverse(); 
...
function isPermutation(list: number[], perms: number[]) : boolean {
  return list.length === perms.length && list.slice().sort((a, b) => a - b).join() === perms.slice().sort((a, b) => a - b).join();
}
...
function pow(x: number, n: number) : number {
  return n === 0 ? 1 : new Array(n).fill(x).reduce((acc, curr) => acc * curr, 1);
}
...
function tetrate(x: number, h: number) : number {
  return h === 0 ? 1 : Array.from({ length: h }, () => x).reduceRight((acc, curr) => curr ** acc);
}
...
function digitsNumberValueInBase(b: number, bDigits: number[]) : number {
  return bDigits.map((digit, index) => digit * b ** index).reduce((acc, curr) => acc + curr, 0);
}
