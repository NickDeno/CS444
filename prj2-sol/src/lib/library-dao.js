"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LibraryDao = void 0;
exports.makeLibraryDao = makeLibraryDao;
var mongo = require("mongodb");
var cs544_js_utils_1 = require("cs544-js-utils");
//TODO: define any DB specific types if necessary
function makeLibraryDao(dbUrl) {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, LibraryDao.make(dbUrl)];
                case 1: return [2 /*return*/, _a.sent()];
            }
        });
    });
}
//options for new MongoClient()
var MONGO_OPTIONS = {
    ignoreUndefined: true, //ignore undefined fields in queries
};
var LibraryDao = /** @class */ (function () {
    //called by below static make() factory function with
    //parameters to be cached in this instance.
    function LibraryDao(client, books) {
        this.client = client;
        this.books = books;
    }
    //static factory function; should do all async operations like
    //getting a connection and creating indexing.  Finally, it
    //should use the constructor to return an instance of this class.
    //returns error code DB on database errors.
    LibraryDao.make = function (dbUrl) {
        return __awaiter(this, void 0, void 0, function () {
            var client, db, books, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        return [4 /*yield*/, new mongo.MongoClient(dbUrl, MONGO_OPTIONS).connect()];
                    case 1:
                        client = _a.sent();
                        db = client.db();
                        books = db.collection('books');
                        // Create text indexes on 'title' and 'authors' fields to support search
                        return [4 /*yield*/, books.createIndex({ title: 'text', authors: 'text' })];
                    case 2:
                        // Create text indexes on 'title' and 'authors' fields to support search
                        _a.sent();
                        // Return a new LibraryDao instance with the connected client and collection
                        return [2 /*return*/, cs544_js_utils_1.Errors.okResult(new LibraryDao(client, books))];
                    case 3:
                        error_1 = _a.sent();
                        return [2 /*return*/, cs544_js_utils_1.Errors.errResult(error_1.message, 'DB')];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    /** close off this DAO; implementing object is invalid after
     *  call to close()
     *
     *  Error Codes:
     *    DB: a database error was encountered.
     */
    LibraryDao.prototype.close = function () {
        return __awaiter(this, void 0, void 0, function () {
            var err_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        // Close the MongoDB client connection
                        return [4 /*yield*/, this.client.close()];
                    case 1:
                        // Close the MongoDB client connection
                        _a.sent();
                        return [2 /*return*/, cs544_js_utils_1.Errors.VOID_RESULT];
                    case 2:
                        err_1 = _a.sent();
                        return [2 /*return*/, cs544_js_utils_1.Errors.errResult(err_1.message, 'DB')];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    //add methods as per your API
    /**
     * Clear out all documents in the 'books' collection.
     */
    LibraryDao.prototype.clear = function () {
        return __awaiter(this, void 0, void 0, function () {
            var err_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, this.books.deleteMany({})];
                    case 1:
                        _a.sent(); // Delete all books
                        return [2 /*return*/, cs544_js_utils_1.Errors.VOID_RESULT];
                    case 2:
                        err_2 = _a.sent();
                        return [2 /*return*/, cs544_js_utils_1.Errors.errResult(err_2.message, 'DB')];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    return LibraryDao;
}()); //class LibDao
exports.LibraryDao = LibraryDao;
