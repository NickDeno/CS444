...
describe('library types', () => {
  ...
  describe('Book validation', () => {
    ...
    it('having no authors is invalid', () => {
      const req = { ...BOOK_1 };
      req.authors = [];  // Set authors to an empty array
      const result = Lib.validate('addBook', req);  // Run validation
      assert(result.isOk === false);  // Expect validation to fail
      expect(result.errors.length).to.be.gt(0);  // Ensure there are validation errors
    });
